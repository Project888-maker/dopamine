...
V1 is intentionally constrained so one trigger produces one small deployable project instead of several overbuilt or truncated apps. By default, V1 prefers browser-based web apps, static tools, and simple API-backed utilities; Telegram bot projects are generated only when the trigger topic explicitly requests Telegram bots.
...
To explicitly allow a Telegram bot project, include Telegram in the topic, for example:

```bash
python trigger_pipeline.py "Telegram bot for developer utilities"
```

The command queues one Celery chain and prints the task id. Watch worker logs until the report step completes, then inspect the newest folder under `runs/`. The report includes the project type, local run command, and required environment variables.
